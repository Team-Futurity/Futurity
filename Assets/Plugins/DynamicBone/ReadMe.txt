...
- Damping
  How much the bones slowed down. 뼈가 얼마나 느려졌는지.

- Elasticity
  How much the force applied to return each bone to original orientation. 각 뼈를 원래 방향으로 되돌리기 위해 가해지는 힘의 양.

- Stiffness
  How much bone's original orientation are preserved. 뼈의 원래 방향이 보존되는 양입니다.


- Inert
  How much character's position change is ignored in physics simulation. 물리 시뮬레이션에서 캐릭터의 위치 변화가 무시되는 정도.

- Friction
  How much the bones slowed down when collide. 충돌할 때 뼈가 느려지는 정도.

- Radius
  Each bone can be a sphere to collide with colliders. Radius describe sphere's size. 각 뼈는 충돌기와 충돌하는 구체가 될 수 있습니다. 반지름은 구체의 크기를 나타냅니다.
